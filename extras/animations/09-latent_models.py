...
class GMMConvergence(Scene):
    """Show how log-likelihood increases with each EM iteration."""
    
    def construct(self):
        # Title
        title = Text("EM Convergence: Log-Likelihood", font_size=36)
        title.to_edge(UP)
        self.play(Write(title))
        ...
        np.random.seed(42)
        data = np.concatenate([
            np.random.normal(-2, 0.8, 40),
            np.random.normal(2.5, 1.0, 35)
        ])
        n = len(data)
        ...
        # Run EM and record log-likelihoods
        log_likelihoods = []
        ...
        for t in range(15):
            ...
            # Log-likelihood
            ll = sum(np.log(sum(pi[k] * stats.norm.pdf(x, mu[k], sigma[k]) for k in range(K))) for x in data)
            log_likelihoods.append(ll)
        
        # Create axes for log-likelihood plot
        axes = Axes(
            x_range=[0, 15, 1],
            y_range=[min(log_likelihoods) - 5, max(log_likelihoods) + 5, 10],
            x_length=10,
            y_length=5,
            axis_config={"color": WHITE},
            tips=False
        ...
        x_label = MathTex("\\text{Iteration}", font_size=28).next_to(axes.x_axis, DOWN)
        y_label = MathTex("\\log L", font_size=28).next_to(axes.y_axis, LEFT, buff=0.1)
        
        self.play(Create(axes), Write(x_label), Write(y_label))
        
        # Animate log-likelihood increasing
        points = VGroup()
        lines = VGroup()
        
        prev_point = None
        for t, ll in enumerate(log_likelihoods):
            point = Dot(axes.c2p(t + 1, ll), color=GREEN, radius=0.08)
            points.add(point)
            
            ll_value = MathTex(f"{ll:.1f}", font_size=20, color=GREEN)
            ll_value.next_to(point, UP, buff=0.1)
            
            if prev_point is not None:
                line = Line(prev_point.get_center(), point.get_center(), color=GREEN, stroke_width=2)
                lines.add(line)
                self.play(Create(line), Create(point), run_time=0.3)
            else:
        ...
        # Add explanation
        explanation = MathTex(
            r"\text{ELBO}(q^{(t+1)}, \theta^{(t+1)}) \geq \text{ELBO}(q^{(t+1)}, \theta^{(t)}) = \log p(\mathbf{X}|\theta^{(t)})",
            font_size=24,
            color=YELLOW
        )
        ...
        convergence_text = Text("Monotonically increasing → guaranteed convergence!", font_size=24, color=BLUE)
        convergence_text.next_to(explanation, UP, buff=0.3)
